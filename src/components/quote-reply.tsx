  ...
  return (
    ...
    >
      <p className="relative text-[15px] leading-relaxed">
        ...
        <span className="text-foreground/95 rounded bg-blue-500/15 border border-blue-500/20 px-0.5 dark:bg-blue-400/25">
          {selection}
        </span>
        <span className="text-foreground/70">{after}</span>
      </p>

      <div className="flex h-9 items-start mt-1">
        {toolbarVisible && (
          <div
            className={cn(
              floating,
              "fade-in zoom-in-95 slide-in-from-top-1 animate-in flex items-center gap-0.5 rounded-lg p-1 duration-200 shadow-sm",
            )}
          >
            {actions.map((action) => {
              ...
              return (
                <button
                  ...
                  className="text-foreground/60 hover:bg-foreground/[0.06] hover:text-foreground/90 flex h-7 items-center gap-1.5 rounded-lg px-2.5 text-[13px] font-medium transition-[background-color,color,scale] duration-150 active:scale-[0.96]"
                >
